...
plt.subplot(3, 2, 3), plt.imshow(gray_image, cmap='gray')
plt.title('GrayScale'), plt.xticks([]), plt.yticks([])

#Plotting blurred images
plt.subplot(3, 2, 2), plt.imshow(cv2.cvtColor(originalBlurredImage, cv2.COLOR_BGR2RGB))
plt.title('5x5 Blur'), plt.xticks([]), plt.yticks([])

plt.subplot(3, 2, 4), plt.imshow(grayBlurredImage, cmap='gray')
plt.title('9x9 Blur'), plt.xticks([]), plt.yticks([])

#Plotting sobel images
plt.subplot(3, 2,5),plt.imshow(sobelHorizontal, cmap='gray')
plt.title('Sobel X'), plt.xticks([]), plt.yticks([])

plt.subplot(3, 2, 6), plt.imshow(sobelVertical, cmap='gray')
