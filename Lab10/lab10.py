...
imgBuilding = cv2.imread('Building.jpg',) 

#Converting image to grayscale
gray_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
gray_image2 = cv2.cvtColor(imgBuilding, cv2.COLOR_BGR2GRAY)
...
imgHarris2 = imgBuilding.copy()
imgShiTomasi2 = imgBuilding.copy()
imgORB2 = imgBuilding.copy()

#Performing corner detection 
dst = cv2.cornerHarris(gray_image, 2, 3, k = 0.04) 
dst = cv2.cornerHarris(gray_image2, 2, 3, k = 0.04) 
...
corners= cv2.goodFeaturesToTrack(gray_image2, 40, 0.01, 10) 
...
#Plotting image ORB
img2 = cv2.drawKeypoints(imgShiTomasi, kp, None, color=(0,255,0), flags=0)
